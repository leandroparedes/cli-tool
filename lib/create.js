...
async function create(moduleName, options) {
    ...
    if (!fs.existsSync(path.join(cwd, ".templates"))) {
        log(chalk.red("The src/templates folder does not exists"));
        process.exit(1);
    }

    if (!fs.existsSync(path.join(cwd, ".templates", "module"))) {
        log(chalk.red("The src/templates/module folder does not exists"));
        process.exit(1);
    }

    if (!fs.existsSync(path.join(cwd, "src", "app"))) {
        log(chalk.yellow("The src/app folder does not exists. Creating it..."));
        fs.mkdirSync(path.join(cwd, "src", "app"));
        log(chalk.green("src/app folder created successfully"));
    }
